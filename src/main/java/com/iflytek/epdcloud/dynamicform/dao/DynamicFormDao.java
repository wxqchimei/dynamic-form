...
import com.iflytek.epdcloud.dynamicform.entity.AttachmentTextAreaField;
...
import com.iflytek.epdcloud.dynamicform.entity.TimeIntervalField;
...
public class DynamicFormDao extends NamedParameterJdbcDaoSupport {
    ...
    public int clearField(String formId) {
        String sql = "UPDATE t_field SET delflag=1 where formId=:formId";
        Map<String, Object> args = new HashMap<>();
        args.put("formId", formId);
        int effectedRows = getNamedParameterJdbcTemplate().update(sql, args);
        return effectedRows;
    ...
    public int addField(Field field) {
        ...
        String sql = "INSERT INTO t_field"
                + "(id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag)"
                + "VALUES(:id,:fieldTypeCode,:formId,:code,:label,:columns,:required,:defaultValue,:sequence,:height,:width,:options,:dateFormat,:maxSize,:validation,:fileNum,:unit,:delflag);";
        ...
        args.put("unit",null);
        args.put("delflag",field.getDelflag());
        switch (field.getFieldType().getCode()) {
            case "datetime":
                ...
                break;
            case "timeInterval":
                TimeIntervalField tif = (TimeIntervalField) field;
                args.put("dateFormat", tif.getDateFormat());
                break;
            case "text":
                TextField tf = (TextField) field;
                args.put("maxSize", tf.getMaxSize());
                args.put("validation", tf.getValidation());
                args.put("unit",tf.getUnit());
                break;
            case "textarea":
            ...
            case "attachmentTextarea":
                AttachmentTextAreaField atf = (AttachmentTextAreaField) field;
                args.put("maxSize", atf.getMaxSize());
                args.put("height", atf.getHeight());
                args.put("width", atf.getWidth());
                args.put("validation", atf.getValidation());
                args.put("fileNum", atf.getFileNum());
                break;
            default:
                break;
        }
    ...
    public List<Field> listField(String formId) {
        String sql =
                "SELECT id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag FROM t_field where formId=:formId order by sequence asc,columns asc";
        Map<String, Object> args = new HashMap<>();
        args.put("formId", formId);
        List<Field> result =
    ...
    public int removeGlobleField(String code) {
        String sql = "DELETE FROM t_field where  `code`=:code";
        Map<String, Object> args = new HashMap<>();
        args.put("code", code);
        int effectedRows = getNamedParameterJdbcTemplate().update(sql, args);
        return effectedRows;
    ...
    public List<FieldValue> listFieldValue(String entityName, String entityId) {
        String sql =
                "SELECT v.id,v.entityName,v.fieldId,v.entityId,v.`key`,v.val FROM t_fieldValue v INNER JOIN t_field f ON v.`fieldId`=f.`id` where v.entityName=:entityName and v.entityId=:entityId ORDER BY f.`sequence`";
        Map<String, Object> args = new HashMap<>();
        args.put("entityName", entityName);
    ...
    public Field getField(String fieldId) {
        String sql =
                "SELECT id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag FROM t_field where id=:fieldId";
        Map<String, Object> args = new HashMap<>();
    ...
    public List<Field> listField(String formId, String sw) {
        String sql =
                "SELECT id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag FROM t_field where formId =:formId and label like '%:sw%' order by sequence asc";
        Map<String, Object> args = new HashMap<>();
        args.put("formId", formId);
        args.put("sw", sw);
        List<Field> result =
    ...
    public List<Field> getFieldsByIds(List<String> fieldIds) {
        String sql =
                "SELECT id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag FROM t_field where id in (:fieldIds)";
     ...
     * @return
     */
    public int updateCopiedField(Field field) {
        StringBuilder sqlbuilder = new StringBuilder(
                "UPDATE t_field SET label=:label,columns=:columns,defaultValue=:defaultValue");
        ...
        sqlbuilder.append(" where id !=:id and code=:code");
    ...
    public int countField(String formId) {
        String sql = "select max(sequence) ct from t_field where  formId=:formId";
        Map<String, String> args = new HashMap<String, String>();
        args.put("formId", formId);
        int res = getNamedParameterJdbcTemplate().queryForInt(sql, args);
        return res;
    ...
    public int getFieldCountByCode(String code) {
        String sql = "select count(0) ct from t_field where  `code`=:code AND delflag = 0";
        Map<String, String> args = new HashMap<String, String>();
        args.put("code", code);
        int res = getNamedParameterJdbcTemplate().queryForInt(sql, args);
        return res;
     ...
     * @return
     */
    public int removeFieldValue(String fieldId) {
        String sql = "DELETE FROM t_fieldValue where fieldId=:fieldId";
        Map<String, Object> args = new HashMap<>();
        args.put("fieldId", fieldId);
        int effectedRows = getNamedParameterJdbcTemplate().update(sql, args);
        return effectedRows;
     ...
     * @return
     */
    public int addFieldBatch(List<String> ids) {
        @SuppressWarnings("unchecked")
        HashMap<String, Object>[] argsArray = new HashMap[ids.size()];
        ...
        String sql = "UPDATE t_field SET delflag=0 WHERE id=:id";
        int[] effectedRows = getNamedParameterJdbcTemplate().batchUpdate(sql, argsArray);
        return effectedRows.length;
    }

    /**
     * 获取模块中delflag为0的field
     * @param formId
     * @return
     */
    public List<Field> listCheckedField(String formId) {
        String sql =
                "SELECT id,fieldTypeCode,formId,code,label,columns,required,defaultValue,sequence,height,width,options,dateFormat,maxSize,validation,fileNum,unit,delflag FROM t_field where formId=:formId AND delflag=0 order by sequence asc,columns asc";
        Map<String, Object> args = new HashMap<>();
        args.put("formId", formId);
        List<Field> result =
                getNamedParameterJdbcTemplate().query(sql, args, RowMapperFactory.fieldRowMapper);
